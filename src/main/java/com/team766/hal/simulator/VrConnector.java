...
public class VRConnector implements Runnable {
	...
	public void process() throws IOException {
		...
		for (PortMapping m : RELAY_CHANNELS) {
			putCommandBool(m.messageDataIndex, ProgramInterface.relayChannels[m.robotPortIndex]);
		}
		...
		if (newData) {
			...
			for (PortMapping m : DIGITAL_CHANNELS) {
				ProgramInterface.digitalChannels[m.robotPortIndex] = getFeedback(m.messageDataIndex);
			}
